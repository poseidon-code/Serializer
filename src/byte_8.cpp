...
serializer::byte_8::byte_8(serializer::Endianness endianness)
    : endianness(endianness)
{}

std::vector<unsigned char> serializer::byte_8::serialize(int64_t value) {
...
void serializer::byte_8::serialize(unsigned char* stream, int64_t value, size_t index_start) {
    this->t.value = value;
    _serialize(stream, this->t.bytes, this->byte_size, index_start, this->endianness);
}

void serializer::byte_8::serialize(std::vector<unsigned char>& stream, int64_t value, size_t index_start) {
    this->t.value = value;
    _serialize(stream.data(), this->t.bytes, this->byte_size, index_start, this->endianness);
}

int64_t serializer::byte_8::deserialize(const unsigned char* stream, size_t index_start) {
    this->t.value = 0;
    _deserialize(stream, this->t.bytes, this->byte_size, index_start, this->endianness);
    return t.value;
}

int64_t serializer::byte_8::deserialize(const std::vector<unsigned char>& stream, size_t index_start) {
    this->t.value = 0;
    _deserialize(stream.data(), this->t.bytes, this->byte_size, index_start, this->endianness);
    return t.value;
}

std::ostream& serializer::operator<<(std::ostream& os, const byte_8& byte) {
    os << "0x ";
